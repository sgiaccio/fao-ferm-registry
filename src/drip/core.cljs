   ...
   [:div {:class "max-w-7xl mx-auto px-4 sm:px-6"}
    ...
    ;;     [:path {:stroke-linecap "round", :stroke-linejoin "round", :stroke-width "2", :d "M4 6h16M4 12h16M4 18h16"}]]]]
     [:div {:class "flex items-center justify-end md:flex-1 lg:w-0"}
      ...
      (when @config/is-admin [:a {:href  (href :admin) :class "ml-8 whitespace-nowrap inline-flex items-center justify-center px-4 py-2 border border-indigo-300 rounded-md shadow-sm text-base font-medium text-black hover:bg-indigo-200"} "Admin"])
