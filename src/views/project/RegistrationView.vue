...
<template>
  <TabTemplate :title="t('projectRegistration.title')">
    ...
    <div
      class="divide-y divide-slate-100 border-2 border-slate-200 rounded-md shadow-sm mt-4 mb-6 overflow-hidden"
    >
      <TextFormGroup
        ...
        <template #info>
          <div>
            ...
            <p class="mt-2">
              {{
                t(
                  "projectRegistration.fields.restorationStatus.info.references.title",
                )
              }}
              <br />
              <a
                :href="
                  t(
                    'projectRegistration.fields.restorationStatus.info.references.linkHref',
                  )
                "
                target="_blank"
              ...
              >
                {{
                  t(
                    "projectRegistration.fields.restorationStatus.info.references.linkText",
                  )
                }}
              </a>
            </p>
            ...
            >
              <option value="1">
                {{
                  t(
                    "projectRegistration.fields.restorationStatus.options.preparation",
                  )
                }}
              </option>
              <option value="2">
                {{
                  t(
                    "projectRegistration.fields.restorationStatus.options.progress",
                  )
                }}
              </option>
              <option value="3">
                {{
                  t(
                    "projectRegistration.fields.restorationStatus.options.monitoring",
                  )
                }}
              </option>
            </select>
            ...
            <p v-if="selectedItemInRestorationStatusInfo === '1'">
              {{
                t(
                  "projectRegistration.fields.restorationStatus.descriptions.preparation",
                )
              }}
            </p>
            <p v-if="selectedItemInRestorationStatusInfo === '2'">
              {{
                t(
                  "projectRegistration.fields.restorationStatus.descriptions.progress",
                )
              }}
            </p>
            <p v-if="selectedItemInRestorationStatusInfo === '3'">
              {{
                t(
                  "projectRegistration.fields.restorationStatus.descriptions.monitoring",
                )
              }}
            </p>
          </div>
      ...
      <RecursiveMenuFormGroup
        ...
        :label="
          store.project.reportingLine === 'GEF'
      ...
      >
        ...
        >
          <p class="font-semibold text-sm text-gray-500 mb-3">
            ...
            <InfoButton
              :title="t('projectRegistration.fields.objectives.label')"
            >
              <template #default>
                <p>
                  {{
                    t(
                      "projectRegistration.fields.objectives.additionalInfo.details",
                    )
                  }}
                </p>
                <p class="mt-2">
                  <b>{{
                    t(
                      "projectRegistration.fields.objectives.additionalInfo.example",
                    )
                  }}</b>
                </p>
              </template>
      ...
      >
        <template #info>
          ...
          <p class="pt-4">
            {{ t("references") }}:
            <br />
            FAO. 2022. Voluntary Guidelines on the Responsible Governance of
            Tenure of Land, Fisheries and Forests in the Context of National
            Food Security. First revision. Rome.
            <a
          ...
          <p class="pt-4">
            FAO, IUCN CEM & SER. (2021). Principles for ecosystem restoration to
            guide the United Nations Decade 2021&mdash;2030. Rome, FAO.
          </p>
        </template>
      </RecursiveMenuFormGroup>
      ...
      <FileUploadFormGroup2
        :label="
          store.project.reportingLine === 'GEF'
            ? t('projectRegistration.fields.uploadInitiativeDocument.gef.label')
            : t('projectRegistration.fields.uploadInitiativeDocument.label')
        "
        :projectId="store.id!"
        folder="documents"
        :multiple="false"
