        ...
        <div class="max-w-3xl mx-auto">
            ...
            <nav aria-label="Section">
                <ol role="list"
                    ...
                    <li v-for="(step, stepIdx) in tabs" :key="step.name" class="relative md:flex md:flex-1">
                        <router-link v-if="route.name === step.routeName" :to="{ name: step.routeName }"
                            class="flex items-center py-4 text-sm font-medium" aria-current="step">
                            <span class="justify-self-start ml-1 text-sm font-medium text-indigo-600">{{ step.name }}</span>
                        </router-link>

                        <router-link v-else :to="{ name: step.routeName }" :key="step.routeName" class="group flex items-center">
            ...
            <router-view v-if="store.project" :edit="true" />

            <!-- <div class="w-full relative">
            <button
                @click="save"
                type="button"
                class="inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 ">Save and leave
            </button>
            ...
            <div class="w-full mb-8 flex gap-x-6">
                <div class="shrink">
                    <button @click="saveAndExit" type="button"
                        class="inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                        Save and exit
                    </button>
                </div>
                <div v-if="!lastTab" class="shrink">
                    <button @click="saveAndNext" type="button"
                        class="inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                        Save and next
                    </button>
                </div>
