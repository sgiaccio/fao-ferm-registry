...
async function uploadFile() {
    ...
    return fetch(
        'https://europe-west3-fao-ferm.cloudfunctions.net/load_shapefile_multiple',
        {
            method: 'POST',
            headers: {
                // TODO
              ...
              <DialogPanel class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                      <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900">Upload shapefile</DialogTitle>
                      <div class="mt-2">
                        <p class="text-sm text-gray-500">Please upload a polygon or multipolygon zip file with all the shapefile files (.shp, .shx, .dbf, ...).</p>
                        <p class="text-sm text-gray-500 mt-2">Please note: one area per feature will be created, i.e. if your shapefile contains n polygons, n new areas will be added</p>
                ...
                <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  ...
                  <button type="button" class="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" @click="emit('cancel')" ref="cancelButtonRef">Cancel</button>
                </div>
              </DialogPanel>
            </TransitionChild>
          </div>
