...
import { getProjectAreas, getProjectAdminAreas, getAggregatedProjectAreasGeoJson } from '@/firebase/functions';
...
onMounted(async () => {
    ...
    const aggregatedPolygons = getAggregatedProjectAreasGeoJson(store.id!, store.project.project.areas.map(a => a.uuid).filter(Boolean));
});
